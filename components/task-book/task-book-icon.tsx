...
export default function TaskBookIcon({ size = 24, onClick, isOpen = false, hasNewTask = false }: TaskBookIconProps) {
    return (
        <div className="relative">
            <div 
                className={`cursor-pointer hover:opacity-60 transition-opacity ${hasNewTask && !isOpen ? 'animate-pulse' : ''}`}
                onClick={onClick}
                style={{
                    ...(hasNewTask && !isOpen && {
            ...
            {hasNewTask && !isOpen && (
                <Badge 
                    variant="default" 
                    className="absolute -top-2 -right-2 text-[10px] px-1.5 py-0 h-5 pointer-events-none"
                >
                    New task
                </Badge>
            )}
